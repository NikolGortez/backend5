...
<body>
	...
	<div class="container">
		<form action="" method="POST">
			<p><label for="name" style="color: #381237;">Имя</label>
			...
			<p><label for="email" style="color: #381237;">E-mail</label>
			...
			<p><label for="year" style="color: #381237;">Год рождения</label>
			...
			<p><label <?php if(!empty($errors['gender'])) print 'class="error_check"'?>style="color: #381237;">Пол:</label>
			...
			<p><label <?php if(isset($_COOKIE['kon_error'])) print 'class="error_check"'?>style="color: #381237;">Количество конечностей:</label>
			...
			<p><label <?php if(!empty($errors['super'])) print 'class="error_check"'?> style="color: #381237;"multiple>Сверхспособности:</label>
			<!-- выводим способности прямо из бд-->
			<p><select name="super[]" multiple="multiple" <?php if(!empty($errors['super'])) print 'class="error"'?> style="color:#381237;">Сверхспособности:</label> 
      <?php 
        $sql = 'SELECT * FROM SuperDef'; 
        foreach ($db->query($sql) as $row) { 
            $selected = (isset($values['super'][$row['id']])&&empty($_COOKIE['super_error'])) ? 'selected' : ''; 
            $id1 = $row['id']; 
            $value1 = $row['name']; 
            print("<option value='$id1' $selected>$value1</option>"); 
        } 
      ?> 
</select>
			<p><label for="bio" style="color: #381237;">Биография</label><br>
			...
			<p><label <?php if(!empty($errors['contr_check'])) print 'class="error_check"'?> style="color: #381237;">С контрактом ознакомлен:</label>
